...
If you were given a task folder, your task prompt names the ranges to review, one per repository of the task — the project and every module or library checkout it touched (`spine-toolkit`, `conventions/task-ranges.md`). Each range is named by its repository's path relative to the project root (`.` is the project itself); run git in `<project root>/<that path>`. Review exactly those ranges:
...
- `nav-multiplatform` — KMP navigation: one library for the whole project (navigation-compose, Navigation 3, Decompose or Voyager), back handling and state preservation per platform
...
- `di-koin` — PR red flags: `KoinComponent` / `by inject()` reached from domain code (Service Locator); `single { }` for a per-screen stateful object; a module changed with nothing checking the graph, as `di-koin` → "Testing" does it; platform bindings declared in `commonMain` rather than a platform module
...
```
[REVIEWED_COMMIT] = .: <sha>
[REVIEWED_COMMIT] = <library checkout>: <sha>
```
