...
@test "the declared core floor reads everything this plugin relies on" {
  # `surfaces` and the unpinned agent models all arrive by 1.12.0; 2.0 held the floor
  # while the reason was the config format — every config this plugin's skills and
  # agents read names `[FIELD] = [value]` lines, and a 1.x core neither writes nor
  # parses them. 2.7.0 raises it: the testers, the reviewer, the developers and
  # diagnostics point at sections of `spine-toolkit:test-authoring` that 2.6.0 does not
  # have all of, and test-discipline.test.bats checks them against the core checkout at
  # this floor.
...
@test "the brief's drive_app and manual_checks outrank every validator's file chain" {
