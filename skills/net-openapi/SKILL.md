...
2. **KMP takes only what compiles in `commonMain`.** `openapi-generator` with `library =
   "multiplatform"` is built for it (Ktor and kotlinx.serialization underneath); no `jvm-*` row is
   (`pkg-kmp-source-sets`). Fabrikt's kotlinx.serialization models and its Ktor client import no JVM
   package once `--validation-library` is `NO_VALIDATION` — the default puts `jakarta.validation`
   annotations on every property — and the spec has nothing that maps to a JVM type: `number`
   becomes `BigDecimal`, `format: uuid` `java.util.UUID`, `format: uri` `java.net.URI`. Its OkHttp,
   OpenFeign and Spring clients are JVM-only.
...
import com.example.api.generated.model.OrderDraft
...
internal class GeneratedOrdersApi(private val generated: Generated) : OrdersApi {
    ...
    override suspend fun orders(cursor: String?): Page<OrderDto> {
        val page = generated.listOrders(cursor = cursor).bodyOrThrow()
        return Page(page.items.map { it.toDto() }, page.next)
    }

    override suspend fun order(id: String): OrderDto = generated.getOrder(id).bodyOrThrow().toDto()

    override suspend fun place(draft: OrderDraftDto, idempotencyKey: String): OrderDto =
        generated.createOrder(idempotencyKey = idempotencyKey, orderDraft = draft.toGenerated())
            .bodyOrThrow().toDto()
}

private fun <T : Any> Response<T>.bodyOrThrow(): T =
    if (isSuccessful) checkNotNull(body()) else throw HttpException(this)

private fun GeneratedOrder.toDto() = OrderDto(id = id, total = total, currency = currency)

private fun OrderDraftDto.toGenerated() = OrderDraft(items = lines)
```

`OrdersApi`, `Page`, `OrderDto` and `OrderDraftDto` are the project's, declared where
`net-architecture` puts them. With `useCoroutines`, `jvm-retrofit2` emits `suspend` functions that
return `Response<T>` (without it, `Call<T>`), and a `Response` never throws on a status: the adapter
turns a non-2xx into Retrofit's `HttpException` and lets it out with every other transport failure
(`net-architecture` → "Core Shape"). The generator puts header parameters before the body — the
reverse of `place` — so the call names its arguments. The generated interface comes from the
project's own `Retrofit` (`retrofit.create<Generated>()`); the generated `ApiClient` builds an
OkHttp client of its own, outside the middleware stack. The mapping looks redundant on the day it is
written and stops being redundant the first time the spec renames a field or adds one the domain
does not want.
...
plugins {
    ...
    alias(libs.plugins.kotlin.serialization)        // the generated models are @Serializable
...
openApiGenerate {
    ...
    inputSpec.set("$projectDir/specs/orders.yaml")  // committed beside the build
...
dependencies {
    // what the generated tree compiles against, its ApiClient included
...
The `srcDir` and `dependsOn` lines are the whole wiring: the output is a source root, and
compilation depends on the task that fills it. Without the `dependsOn` the first clean build compiles
against nothing and the error names a missing package rather than a missing task. Without the
serialization plugin the build stays green: `@Serializable` on the generated models then produces no
serializer, and the first call fails with Retrofit's "Unable to create converter for class …",
caused by "Serializer for class 'Order' is not found".
...
| Ktor | `io.ktor:ktor-server-routing-openapi` builds the spec from the routing tree at runtime, from `describe { }` on a route (`@ExperimentalKtorApi`) and, with the Ktor Gradle plugin's `ktor { openApi { enabled = true } }` on Kotlin 2.4+, from what its compiler plugin infers out of handler code and KDoc; `ktor-server-openapi` and `ktor-server-swagger` serve it | the natural fit: `openapi-generator` `generatorName = "kotlin-server"` with `library = "ktor"`, or hand-written routes and a test that validates them against the spec |
...
| Micronaut | `micronaut-openapi` — an annotation processor writes the spec at compile time | the `io.micronaut.openapi` Gradle plugin runs the other direction: `micronaut { openapi { server(file("orders.yaml")) { lang = "kotlin" } } }`, and `client(...)` for the consuming side |
...
3. **Ktor derives the spec at runtime, never in the build.** Its Gradle plugin no longer writes a
   file (`openApi.target` is deprecated), so rule 2 applies: a `testApplication` test reads the
   served spec and writes or compares the committed file. And `openAPI(...)` and `swaggerUI(...)`
   with no file argument serve `openapi/documentation.yaml` whenever that resource exists and fall
   back to the routes only when it does not — a stale hand-written file wins silently.
...
5. **Swagger UI served over a spec nobody maintains.** `ktor-server-swagger` handed a file renders
   that file, and one left at `openapi/documentation.yaml` outranks the routes, so the documentation
   endpoint reports, with full confidence, whatever the API looked like in the sprint the file was
   last touched.
