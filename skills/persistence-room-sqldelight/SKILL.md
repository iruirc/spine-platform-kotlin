...
| Kotlin Multiplatform, starting fresh | SQLDelight | it was multiplatform first: `commonMain` schema, a driver per target including `WebWorkerDriver`, and no annotation processor to run on Kotlin/Native |
...
| A Room codebase whose target list includes web | Room 3 (`androidx.room3`) | the only Room line with JS and Wasm targets; its package differs, so the move is a migration of every import |
...
1. **Room 2.x's target list is Android, iOS, JVM/desktop and native — not web.** Room 3
   (`androidx.room3`, stable since July 2026) adds JS and Wasm under a new package; the samples
   here stay on Room 2.8, the version most projects are on.
...
6. **KSP, not kapt.** `ksp("androidx.room:room-compiler:<version>")` beside
   `implementation("androidx.room:room-runtime:<version>")`, which carries `withTransaction` and the
   coroutine extensions; `androidx.room:room-ktx` has been an empty artifact since 2.7.0.
...
7. **SQLite has no date, no boolean and no enum**, so the schema declares an adapter:
   ...
   of the file, the `ColumnAdapter` passed to the database constructor. `AS Int` is no exception: it
   needs `import kotlin.Int;` and `IntColumnAdapter` from `app.cash.sqldelight:primitive-adapters`.
   Keeping that mapping in one place is what stops half the codebase parsing timestamps by hand.
8. **Schema changes are `.sqm` files** — `persistence-migrations` → "SQLDelight".
...
3. **Room, across DAOs or with logic between the writes: `db.withTransaction { }`** from
   `room-runtime`. It is `suspend`, it is coroutine-aware, and it is the only correct way to hold a
...
- **Decide the destructive fallback before the first release.** When it may be used, and what ships
  instead, is `persistence-migrations` → "Room".
...
2. **The destructive fallback in a release build.** What it costs the user, and the migration
   that replaces it, is `persistence-migrations` → "Room".
